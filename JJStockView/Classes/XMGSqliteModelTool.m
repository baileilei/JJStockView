...
+ (BOOL)saveOrUpdateModel:(id)model uid:(NSString *)uid {
    ...
    // 1. 判断表格是否存在, 不存在, 则创建
    if (![XMGTableTool isTableExists:cls uid:uid]) {
    ...
    NSArray *result = [XMGSqliteTool querySql:checkSql uid:uid];
    
    
    // 获取字段名称数组
    NSArray *columnNames = [XMGModelTool classIvarNameTypeDic:cls].allKeys;
    ...
    return [XMGSqliteTool deal:execSql uid:uid];
}
